...
export default function MessageList({ type = 'inbox' }) {
  ...
  if (loading) return <div className="flex justify-center p-8">Carregando mensagens...</div>;
  if (error) return <div className="text-red-500 p-4">{error}</div>;
  if (messages.length === 0) return <div className="p-4 text-gray-500">Nenhuma mensagem encontrada</div>;
      ...
      {messages.map((message) => (
        <div key={message._id} className="p-4 hover:bg-gray-50 transition">
          ...
          {!message.read && type === 'inbox' && (
            <span className="inline-block mt-2 px-2 py-0.5 text-xs bg-blue-100 text-blue-800 rounded-full">
              Não lida
            </span>
          )}
        </div>
